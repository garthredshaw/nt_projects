-- Archived Jobs Application Snapshot (Date Range)
-- 20150707
SET NOCOUNT ON
SET DATEFORMAT DMY
...
0 AS 'TotalJobTAT',
(
	SELECT MIN(RWH4.dteLandingDate) 
	FROM refRequisitionWorkflowStep RWS4
	JOIN relRequisitionWorkflowHistory RWH4
	ON RWS4.uidId = RWH4.uidRequisitionWorkflowStepId
	WHERE RWS4.uidId IN 
	(
		SELECT uidId FROM refRequisitionWorkflowStep WHERE nvcName = 'Review'
	)
	AND RWH4.uidRequisitionId = REQ1.uidId
) AS 'DateJobFirstReview',
(
	SELECT MIN(RWH3.dteLandingDate) 
	FROM refRequisitionWorkflowStep RWS3
	JOIN relRequisitionWorkflowHistory RWH3
	ON RWS3.uidId = RWH3.uidRequisitionWorkflowStepId
	WHERE RWS3.uidId IN 
	(
		SELECT uidId FROM refRequisitionWorkflowStep WHERE nvcName = 'Archived'
	)
	AND RWH3.uidRequisitionId = REQ1.uidId
) AS 'DateJobArchived',
(
	SELECT COUNT(uidId) 
	FROM relApplication
...
(
	...
	WHERE CAST(FLOOR(CAST(DateJobArchived AS FLOAT))AS DATETIME) >= '@Toate'
	AND CAST(FLOOR(CAST(DateJobArchived AS FLOAT))AS DATETIME) <= '@FromDate'
)
