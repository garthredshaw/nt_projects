...
(
	...
	JOIN relApplication APP ON APP.uidCandidateId = C.uidId	AND APP.uidRequisitionId = @uidRIdParam
	WHERE
	(
		C.uidWebsiteId IN 
		(
			SELECT uidWebsiteId 
			FROM relWebsitePermission 
			WHERE enmPermission = 1 
			AND uidRoleId IN 
			(
				SELECT uidRoleId 
				FROM relRoleMembership 
				WHERE uidUserId = @uidUIdParam
			)
		)
	)
...
EXEC sp_executeSql @nvcSql, N'@uidLIdParam uniqueidentifier, @uidUIdParam uniqueidentifier, @uidRIdParam uniqueidentifier', @uidLIdParam = @uidLanguageId, @uidUIdParam = @uidUserId, @uidRIdParam = @uidRequisitionId