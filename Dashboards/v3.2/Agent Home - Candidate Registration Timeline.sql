...
FROM 
	...
	(
		SELECT
			MONTH(dteRegistrationDate) as intMonth,
			YEAR(dteRegistrationDate) intYear,
			COUNT(*) as intRegistrations
		FROM
			dtlCandidate
		WHERE
			dteRegistrationDate > '1 ' + DATENAME(month, (DATEADD(month, 0-@intPeriod, GETDATE()))) + ' ' + CAST(YEAR(DATEADD(month, 0-@intPeriod, GETDATE())) as nvarchar)
			AND
			(
				uidId IN (SELECT uidCandidateId FROM relAgencyCandidate WHERE uidAgencyId = @uidAgencyId)
			)
		GROUP BY
			MONTH(dteRegistrationDate), YEAR(dteRegistrationDate)
